...
var verifyreCaptcha = function verifyreCaptcha(response) {
    $.post('/reCaptCha', { 'g_recaptcha_response': response }).done(function (data) {
        switch (data.code) {
            case -1:
                showNotification('', 'Verification failed. Try again.', 'danger');
                break;
            case -2:
                showNotification('', 'Please wait a bit longer to verify again.', 'danger');
    ...
    restartVerification = function restartVerification() {
    if (timeLeft > 0) {
        timeLeft -= 1;
