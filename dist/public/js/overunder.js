...
var ouVM = $.extend({}, baseVM, autoBetVM, {
    ...
    //functions
    inc: function inc() {
        ouVM.payout(ouVM.payout() + 0.5);
    },
    dec: function dec() {
        ouVM.payout() >= 2.5 ? ouVM.payout(ouVM.payout() - 0.5) : 2;
    },
    clickBet: function clickBet(d) {
    ...
    submitBet: function submitBet(d) {
...
var overunder = function overunder(socket) {
    this.socket = socket;
...
overunder.prototype.rollResult = function () {
    this.socket.on('rollResult', function (result) {
        switch (result.code) {
            case -1:
                showNotification('Fund', 'Fund not enough', 'danger');
                break;
            case -2:
                showNotification('Wager', 'Wager invalid', 'danger');
                break;
            case -3:
                showNotification('Wager', 'Wager has to be a valid number', 'danger');
                break;
        }
        ouVM.betted(false);
    });
...
overunder.prototype.allBets = function () {
    ...
    this.socket.on('allBets', function (roll) {
        ...
        if (roll.amount >= ouVM.highRollers()) self.addToBetHistory(highRollArray, roll);
    });
};
...
overunder.prototype.getAllBets = function () {
    ...
    this.socket.on('getAllBets', function (result) {
        $(result).each(function (i) {
            ...
            if (result[i].amount >= ouVM.highRollers()) self.addToBetHistory(highRollArray, result[i], 1);
        });
    });
};
...
overunder.prototype.showResult = function (result) {
    var win = true;
    if (result.selNum * 1 <= 49.5 && result.rollNum * 1 <= result.selNum * 1 || result.selNum * 1 >= 50.5 && result.rollNum * 1 >= result.selNum * 1) {
    ...
    if (ouVM.numberofRolls() > 1) {
        var stop = false;
        if (win) {
            if (ouVM.stopWin() * 1 > 0 && ouVM.betAmount() >= ouVM.stopWin()) {
                stop = true;
            ...
            if (ouVM.stopLoss() * 1 > 0 && ouVM.betAmount() >= ouVM.stopLoss()) {
                stop = true;
...
overunder.prototype.addToBetHistory = function (betsArray, result, reverse) {
    ...
    if (betsArray().length > 100) betsArray().pop();
    if (!reverse) betsArray.unshift(this.convertBetResult(bet));else betsArray.push(this.convertBetResult(bet));
};
