...
var baseVM = {
    ...
    switchCoins: function switchCoins(coinName) {
        baseVM.coinName(coinName);
        setCoin();
        switch (coinName) {
            case 'BTC':
                this.betAmount(0.00000001.toFixed(8));break;
            case 'NXT':
                this.betAmount(1);break;
        }
    },
    amountX2: function amountX2() {
        var amount = this.betAmount();
        this.betAmount((amount * 2).toFixed(8));
    },
    amountDiv2: function amountDiv2() {
        var amount = this.betAmount();
        this.betAmount((amount / 2).toFixed(8));
    },
    ...
    saveWithdrawalAddress: function saveWithdrawalAddress() {
        socket.emit('withdrawalAddress', baseVM.coinName(), baseVM.withdrawalAddress());
    },
    saveClientSalt: function saveClientSalt() {
    ...
    copyBalance: function copyBalance() {
...
var autoBetVM = {
    ...
    incRolls: function incRolls() {
        this.numberofRolls(this.numberofRolls() + 1);
    },
    decRolls: function decRolls() {
        this.numberofRolls() - 1 >= 0 ? this.numberofRolls(this.numberofRolls() - 1) : 0;
    }
};
...
function registerSocketEvents() {
    ...
    socket.on('withdrawalAddress', function (result) {
        if (result) showNotification('', 'Withdrawal address has been updated.', 'success');else showNotification('', 'Update withdrawal address failed.', 'danger');
    });
    ...
    socket.on('getBalance', function (result) {
        if (result.balance >= 0) {
            ...
            if (baseVM.balance() != result.balance) showNotification('', 'Balance updated.', 'success');
        } else {
...
function setUser(user) {
    ...
    setCoin();
    if (onlineUsersArray) {
        if (onlineUsersArray.indexOf(user) == -1) onlineUsersArray.push(user);
    }
}
    ...
    if (baseVM.funds && baseVM.funds.length > 0) {
        $(baseVM.funds).each(function (i) {
            ...
            if (baseVM.coinName() == f.coinName) {
                ...
                if (f.depositAddress != '') setInterval(getBalance, 30 * 1000); //refresh balance every 30 seconds for deposits
